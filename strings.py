...
def prevod_data_na_databazovy_format(datum_str: str) -> str:
    den, mesic, rok = datum_str.split(". ")
    return f"{rok}-{mesic.zfill(2)}-{den.zfill(2)}"

def vytvor_identifikator(s: str, typ_identifikatoru: str) -> str:
    s = (s.lower()
    ...
    if typ_identifikatoru == "python":
        return "_".join(slova)
    elif typ_identifikatoru == "js":
        return slova[0] + "".join(slovo.capitalize() for slovo in slova[1:])
    else:
        raise ValueError("Neplatný typ identifikátoru!")

def generuj_nahodna_hesla(pocet: int) -> list:
    hesla = []
    velka_pismena = string.ascii_uppercase
    mala_pismena = string.ascii_lowercase
    specialni_znaky = "-+*"
    cislice = string.digits

    for _ in range(pocet):
        heslo = (
            ''.join(random.choice(velka_pismena) for _ in range(3)) +
            ''.join(random.choice(mala_pismena) for _ in range(3)) +
            random.choice(specialni_znaky) +
            ''.join(random.choice(cislice) for _ in range(3))
        )
        hesla.append(heslo)

    return hesla

ukazkove_datum = "12. 10. 2020"
ukazkovy_text = "To je proměnná v Pythonu"
pocet_hesel = 5

prevedene_datum = prevod_data_na_databazovy_format(ukazkove_datum)
python_identifikator = vytvor_identifikator(ukazkovy_text, "python")
js_identifikator = vytvor_identifikator(ukazkovy_text, "js")
vygenerovana_hesla = generuj_nahodna_hesla(pocet_hesel)

prevedene_datum, python_identifikator, js_identifikator, vygenerovana_hesla
